...
import com.ireddragonicy.gamespace.ui.designsystem.PanelTokens
...
internal val ACCENT_PRESETS = PanelTokens.AccentPresets.toTypedArray()
...
/** 0 = Material You, 1..size = preset, size+1 = custom user color. */
val PANEL_COLOR_MODE_CUSTOM: Int get() = PanelTokens.ColorModeCustom

/** Resolves the panel accent color based on panelColorMode setting. */
@Composable
fun rememberPanelAccent(panelColorMode: Int, customColorArgb: Int = 0): Color {
    return when {
        panelColorMode == 0 -> MaterialTheme.colorScheme.primary
        panelColorMode == PANEL_COLOR_MODE_CUSTOM -> Color(customColorArgb)
        else -> ACCENT_PRESETS.getOrElse(panelColorMode - 1) { ACCENT_PRESETS[0] }
    }
}

/**
 * Foreground yang selalu terbaca di atas [bg] solid (chip / tombol filled).
 */
fun readableOn(bg: Color): Color {
    val lum = 0.299f * bg.red + 0.587f * bg.green + 0.114f * bg.blue
    return if (lum > 0.4f) Color.Black else Color.White
}
