...
object XiaomiTouchFeatureClient {
    ...
    /** @section Touch Control Modes — these match the kernel enum in
     *  touch-drivers/include/uapi/touch/xiaomi_touch.h exactly. Do not guess
     *  these values; a wrong id silently lands in the driver's `default:` branch
     *  and only shows up as a LOG_ERROR in dmesg. */
    const val MODE_GAME_MODE = 0                    // Touch_Game_Mode — gates the 240Hz path
    ...
    const val MODE_EXPERT_PRESET = 6                // Touch_Expert_Mode (1..3, default 1)
    const val MODE_TUNE_EDGE_FILTER = 7             // Edge filter (default 2, 0 to 3)
    const val MODE_PANEL_ORIENTATION = 8            // Panel orientation (0..3)
    const val MODE_SUPER_REPORT_RATE = 202          // Touch_Super_Report — see setReportRateBoost

    /**
     * Touch_Report_Rate — the panel's live scan rate in Hz.
     *
     * Written by the odm HAL, not by us: it recomputes this whenever a gating
     * mode changes and pushes the result to `nvt_xm_htc_set_report_rate`.
     * Reading it back is the only truthful answer to "what rate am I at" —
     * inferring it from the gating modes gets the rule wrong (verified on
     * NT36532: Touch_Game_Mode=1 with Touch_Super_Report=0 already reports 240).
     */
    const val MODE_REPORT_RATE = 1011
    ...
    /**
     * `Touch_Mode_NUM` from `uapi/touch/xiaomi_touch.h`.
     *
     * The driver's `nvt_set_cur_value` only records a mode when
     * `mode < Touch_Mode_NUM`; anything above falls through every branch and
     * returns success without doing a thing. That is the dividing line between
     * a control that works and a control that merely *looks* like it works.
     */
    private const val KERNEL_MODE_NUM = 38

    /**
     * Modes above the kernel enum that are nonetheless acted on, because the
     * odm HAL consumes them itself rather than passing them down.
     *
     * Verified on onyx (NT36532) by writing each mode and watching
     * `xiaomi_touch_set_mode_value` / `nvt_xm_htc_set_report_rate` in dmesg:
     * only the report-rate pair produced any hardware effect. The SuperCore,
     * hot-area and vibrator modes are HyperOS features that need Joyose and
     * the MiuiTouch stack, neither of which exists on this ROM.
     */
    private val HAL_CONSUMED_MODES = setOf(
        MODE_SUPER_REPORT_RATE,
        MODE_REPORT_RATE,
        MODE_GAME_START,
        MODE_GAME_PACKAGE_NAME,
    )

    /**
     * Whether writing [modeId] can actually change anything on this device.
     *
     * Used to keep dead controls out of the UI: the HAL returns success for
     * every mode it does not implement, so without this a toggle silently does
     * nothing and the user is left wondering which of them are real.
     */
    fun isModeSupported(modeId: Int): Boolean =
        modeId < KERNEL_MODE_NUM || modeId in HAL_CONSUMED_MODES
