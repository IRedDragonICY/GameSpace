...
import com.ireddragonicy.gamespace.gamebar.DanmakuServiceListener
...
class DanmakuService @Inject constructor(
    ...
    override fun showNotificationAsOverlay(danmakuText: String) {
        if (notificationOverlay.parent == null) {
            notificationOverlay.alpha = 0f
            notificationOverlay.text = danmakuText
            windowManager.addView(notificationOverlay, layoutParams)
            pushNotification()
        } else {
            notificationStack.add(danmakuText)
        }
    }
