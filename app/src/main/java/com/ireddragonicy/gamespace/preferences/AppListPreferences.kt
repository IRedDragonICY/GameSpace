...
class AppListPreferences @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) :
    ...
    /**
     * Build summary dari PerAppSettingStore (typed reader, bukan re-parse JSON).
     * Thermal tetap dari Settings.System karena bukan milik store.
     */
    private fun describeGameConfig(packageName: String): String {
        val store = com.ireddragonicy.gamespace.data.PerAppSettingStore(context)
        val features = buildList {
            // Thermal profile (dari Settings.System — bukan milik store)
            try {
                val json = Settings.System.getStringForUser(
                    context.contentResolver, "mithermal_app_profiles",
                    UserHandle.USER_CURRENT
                )
                if (json != null) {
                    val obj = org.json.JSONObject(json)
                    if (obj.has(packageName)) {
                        val idx = obj.getInt(packageName)
                        val names = com.ireddragonicy.gamespace.thermal.ThermalProfiles.PROFILE_NAMES
                        if (idx in names.indices && names[idx].isNotEmpty()) {
                            add(names[idx])
                        }
                    }
                }
            ...
            // AFME
            if (store.afmeEnabled(packageName)) {
                add("AFME ${store.afmeMultiplier(packageName)}×")
            }
            // Resolution
            val res = store.resolution(packageName)
            if (res != "1.0") add("Res ${res}×")
            // VRS
            if (store.vrsLevel(packageName) > 0) add("VRS")
            // Color enhance
            if (store.colorEnhance(packageName)) add("Color+")
            // SGSR
            when (store.sgsrMode(packageName)) {
                1 -> add("SGSR1")
                2 -> add("SGSR2")
                3 -> add("MobFGSR")
            }
