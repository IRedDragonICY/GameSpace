...
class MonitorSettings @Inject constructor(@ApplicationContext context: Context) {
    ...
    private val _thread = mutableStateOf(prefs.getBoolean("thread_monitor_enabled", false))
    var isThreadMonitorEnabled: Boolean
        get() = _thread.value
        set(value) { _thread.value = value; prefs.edit().putBoolean("thread_monitor_enabled", value).apply() }
    ...
    fun isAnyMonitorEnabled(): Boolean =
        isClassicalMonitorEnabled || isMiniMonitorEnabled || isProcessesMonitorEnabled || isThreadMonitorEnabled || isTempMonitorEnabled
    ...
    var opacity: Float
        get() = prefs.getFloat("monitors_opacity", 0.85f)
        set(value) = prefs.edit().putFloat("monitors_opacity", value).apply()
