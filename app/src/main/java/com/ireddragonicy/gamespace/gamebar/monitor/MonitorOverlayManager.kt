...
class MonitorOverlayManager(
    ...
    fun updateWindowParams() {
        ...
        MONITORS.forEach { def ->
            updateMonitor(owner, isPinned, def.key, def.enabled(settings)) { onDrag, onEnd ->
                DraggableWidget(isPinned, onDrag, onEnd) { def.content(telemetry, settings) }
            }
        }
    }
