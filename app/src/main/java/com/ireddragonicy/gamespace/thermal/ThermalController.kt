...
class ThermalController @Inject constructor(
    ...
    private var appliedMaxUa = ThermalProfiles.MAX_WATT * ThermalProfiles.UA_PER_WATT
    ...
    private fun init() {
        ...
        SystemProperties.set(
            ...
            (chargeMaxWatt * ThermalProfiles.UA_PER_WATT).toString()
        )
        SystemProperties.set(
     ...
     *
     * IPC with the Rust daemon uses chunked properties (values are capped at
     * 92 bytes). Chunks and count are written BEFORE the seq bump so the
     * daemon can never observe a torn payload: it only re-reads after seq
     * changes.
     */
    private fun applyCustomProfileByIndex(arrayIndex: Int): Boolean {
        ...
        return try {
            ...
            if (arrayIndex < 0 || arrayIndex >= profiles.length()) return false
            val profile = profiles.getJSONObject(arrayIndex)
            val clusters = profile.getJSONArray("clusters").toString()

            val chunks = clusters.chunked(ThermalProfiles.PROP_CHUNK_SIZE)
            if (chunks.size > ThermalProfiles.PROP_CHUNK_MAX) {
                Log.e(TAG, "Custom profile too large: ${clusters.length} bytes")
                return false
            }
            ...
            Log.i(
                TAG, "Custom profile '${profile.optString("name")}'" +
                    " (${clusters.length} bytes → ${chunks.size} prop chunks)"
            )
            true
        } catch (e: JSONException) {
    ...
    private fun applyChargeSpeed() {
        val maxUa = chargeMaxWatt * ThermalProfiles.UA_PER_WATT
        val minUa = chargeMinWatt * ThermalProfiles.UA_PER_WATT
        ...
        if (maxUa != appliedMaxUa) {
            KernelNodeIO.write(ThermalNodes.WIRED_CHG_CURR, maxUa)
            KernelNodeIO.write(ThermalNodes.WIRED_CHG_CURR2, maxUa)
            appliedMaxUa = maxUa
            Log.d(TAG, "Charge range: $chargeMinWatt-$chargeMaxWatt W")
        }
    }
        ...
        context.registerReceiver(
            object : BroadcastReceiver() {
                override fun onReceive(ctx: Context, intent: Intent) {
                    // Force re-apply after PD negotiation settles
                    handler.postDelayed({
                        Log.i(TAG, "Charger connected: invalidating maxUa cache")
                        appliedMaxUa = -1 // Invalidate cache
                        applyChargeSpeed()
                    }, 2000)
                }
            },
