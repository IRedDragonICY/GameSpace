...
object ThermalProfiles {
    ...
    /**
     * The built-in scenes worth offering next to a game: Auto, the gaming and
     * performance scenes, and Battery Saver. The camera/video/call scenes are
     * picked by the system, not by a player, so the compact pickers (hub chip,
     * panel header) hide them.
     */
    val GAMING_INDICES = listOf(0, 2, 3, 4, 5, 6, 7, 16)
    ...
    // Watt <-> µA conversion lives in ChargingProfiles — the single converter.
    const val MIN_WATT = 5
    const val MAX_WATT = 90
    const val DEFAULT_TEMP_LIMIT_DECI_C = 380 // 38.0°C

    // ── Single parser for custom profiles (eliminates 4x duplication) ──

    data class CustomProfileEntry(val index: Int, val name: String, val id: String)

    /**
     * Parse custom profiles from JSON. All consumers use this.
     * O(n) single pass, no duplicate parsing.
     */
    fun parseCustomProfiles(json: String?): List<CustomProfileEntry> {
        ...
        return try {
            ...
            (0 until arr.length()).map { i ->
                ...
                CustomProfileEntry(
                    ...
                    name = obj.optString("name", "Custom ${i + 1}"),
                    id = obj.optString("id", ""),
                )
            }
    ...
    /** One selectable entry in any thermal-profile picker. */
    data class ProfileOption(val index: Int, val name: String) {
        val isCustom get() = index >= CUSTOM_PROFILE_BASE
    }

    /**
     * The picker list: built-in scenes + user-created custom profiles.
     *
     * The single builder for every surface that offers a thermal choice (global
     * screen, per-app editor, hub chip, panel header) — those used to keep four
     * hand-rolled copies of this list, which is how index 3/4 ended up labelled
     * "Heavy Gaming"/"Light Gaming" in one of them while [PROFILE_SCONFIG] maps
     * them to MGAME/TGAME, i.e. Balanced/Heavy.
     *
     * @param firstName renames index 0, which means different things by scope:
     *   globally it is "Auto" (leave the vendor scene alone), per-app it is
     *   "Follow Global" (inherit whatever the global profile picked).
     * @param builtinIndices restricts the built-in half; the hub chip only
     *   offers the gaming-relevant subset.
     */
    fun options(
        customJson: String?,
        firstName: String? = null,
        builtinIndices: List<Int> = PROFILE_NAMES.indices.toList(),
    ): List<ProfileOption> =
        builtinIndices.map { i ->
            ProfileOption(i, if (i == 0 && firstName != null) firstName else PROFILE_NAMES[i])
        } + parseCustomProfiles(customJson).map { ProfileOption(it.index, it.name) }

    /** Display name for [index], custom profiles included. */
    fun nameOf(index: Int, customJson: String?): String = when {
        index in PROFILE_NAMES.indices -> PROFILE_NAMES[index]
        else -> parseCustomProfiles(customJson).find { it.index == index }?.name ?: "Custom"
    }
}
